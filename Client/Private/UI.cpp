...
CUI::CUI(ID3D11Device * pDevice, ID3D11DeviceContext * pDevice_Context)
...
HRESULT CUI::NativeConstruct_Prototype(const CTransform::TRANSFORMDESC & TransformDesc)
{
...
CUI * CUI::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext, const CTransform::TRANSFORMDESC & TransformDesc)
{
	CUI*		pInstance = new CUI(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype(TransformDesc)))
	{
		MSGBOX(TEXT("Failed to CUI"));
...
CGameObject * CUI::Clone(void * pArg)
{
	...
	{
		MSGBOX(TEXT("Failed to Cloned : CUI"));
		Safe_Release(pInstance);
	}
	return pInstance;
}

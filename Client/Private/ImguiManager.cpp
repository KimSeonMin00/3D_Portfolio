...
void CImguiManager::Tick(_double TimeDelta)
{
	...
	if(ImGui::Begin("Tool_List"))
	{
		...
		if (ImGui::Button("Monster"))
		{
			m_iCurrentItemIndex = 0;
			m_eToolList = TOOL_MONSTER;
		}
...
void CImguiManager::Monster_Tool()
{
	ImGui::Text("Monster_Number");
	ImGui::PushItemWidth(100);
	ImGui::InputInt("MapObject_Index", &m_iMonsterIndex, 1, 10);
	if (m_iMonsterIndex >= m_iNumMonsterIndex)
		m_iMonsterIndex = m_iNumMonsterIndex - 1;

	if (ImGui::Button("Create"))
	{
		if(m_iMonsterIndex == 0)
			m_pGameInstance->Add_Layer(LEVEL_GAMEPLAY, TEXT("Layer_Monster"), TEXT("Prototype_GameObject_Wolf"));

		else if (m_iMonsterIndex == 1)
			m_pGameInstance->Add_Layer(LEVEL_GAMEPLAY, TEXT("Layer_Monster"), TEXT("Prototype_GameObject_RazorBeak"));

		else if (m_iMonsterIndex == 2)
			m_pGameInstance->Add_Layer(LEVEL_GAMEPLAY, TEXT("Layer_Monster"), TEXT("Prototype_GameObject_Red"));
		
		m_iNumMonster++;
		m_iCurrentItemIndex = m_iNumMonster - 1;
	}
	...
	if (ImGui::Button("Delete"))
	{
		if (m_iNumMonster > 0)
		{
			CGameObject* pGameObject = m_pGameInstance->Get_GameObjectPtr(LEVEL_GAMEPLAY, TEXT("Layer_Monster"), m_iCurrentItemIndex);
	...
	{
		char Item[256] = "";
		sprintf_s(Item, "Monster_%d", m_iCurrentItemIndex);

		CurrentItem = Item;


		if (ImGui::BeginCombo("Monster", CurrentItem))
		{
			for (int n = 0; n < m_iNumMonster; n++)
			{
				char Item[256] = "";
				sprintf_s(Item, "Monster_%d", n);
		...
		((CMonster*)m_pGameInstance->Get_GameObjectPtr(LEVEL_GAMEPLAY, TEXT("Layer_Monster"), m_iCurrentItemIndex))->Set_Select();

		_float3 fRotation = { 0.f ,0.f ,0.f };
		_float3 fPosition = { 0.f, 0.f, 0.f };

		m_pTransform = (CTransform*)m_pGameInstance->Get_Component(LEVEL_GAMEPLAY, TEXT("Layer_Monster"), TEXT("Com_Transform"), m_iCurrentItemIndex);
							...
							if (SUCCEEDED(hr))
							{
								...
								if (SUCCEEDED(hr))
								{
									...
									if (SUCCEEDED(hr))
									{
										...
										for (_uint i = 0; i < m_iNumMonster; i++)
										{
											CMonster* pMonster = (CMonster*)m_pGameInstance->Get_GameObjectPtr(LEVEL_GAMEPLAY, TEXT("Layer_Monster"), i);
											if (pMonster != nullptr)
											{
												Safe_AddRef(pMonster);
												m_pTransform = (CTransform*)m_pGameInstance->Get_Component(LEVEL_GAMEPLAY, TEXT("Layer_Monster"), TEXT("Com_Transform"), i);
												if (nullptr != m_pTransform)
												{
													_uint	iMonsterIndex;
													_float4x4 WorldMat;

													iMonsterIndex = pMonster->Get_Index();

													Safe_AddRef(m_pTransform);
													XMStoreFloat4x4(&WorldMat, m_pTransform->Get_WorldMatrix());
													Safe_Release(m_pTransform);

													WriteFile(hFile, &iMonsterIndex, sizeof(_uint), &dwByte, nullptr);
													WriteFile(hFile, &WorldMat, sizeof(_float4x4), &dwByte, nullptr);
												}
								...
								if (SUCCEEDED(hr))
								{
									...
									if (SUCCEEDED(hr))
									{
										...
										while (true)
										{
											...
											ReadFile(hFile, &iMonsterIndex, sizeof(_uint), &dwByte, nullptr);
											ReadFile(hFile, &WorldMat, sizeof(_float4x4), &dwByte, nullptr);
											...
											m_pTransform = (CTransform*)m_pGameInstance->Get_Component(LEVEL_GAMEPLAY, TEXT("Layer_Monster"), TEXT("Com_Transform"), m_iNumMonster);
											Safe_AddRef(m_pTransform);

...
HRESULT CWolf::NativeConstruct_Prototype(const CTransform::TRANSFORMDESC & TransformDesc)
{
...
HRESULT CWolf::NativeConstruct(void * pArg)
{
...
HRESULT CWolf::SetUp_Components()
{
	/* For.Com_Renderer */
...
void CWolf::Change_State(_float fTimeDelta)
{
...
void CWolf::Check_Loop(_float fTimeDelta)
{
	switch (m_eState)
	{
	case STATE_IDLE:
...
void CWolf::Update_State(_float fTimeDelta)
{
	switch (m_eState)
	{
	case STATE_IDLE:
...
void CWolf::Idle(_float fTimeDelta)
{
...
void CWolf::Attack(_float fTimeDelta)
{
	if (m_bStateChange == true)
	{		
...
void CWolf::Death(_float fTimeDelta)
{
	if (m_bStateChange == true)
	{
		m_iCurrentIndex = 6;
...
CWolf * CWolf::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext, const CTransform::TRANSFORMDESC & TransformDesc)
{
	CWolf*		pInstance = new  CWolf(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype(TransformDesc)))
	{
		MSGBOX(TEXT("Failed to Created :  CWolf"));
...
CGameObject * CWolf::Clone(void * pArg)
{
	...
	{
		MSGBOX(TEXT("Failed to Cloned : CWolf"));
		Safe_Release(pInstance);
	}
	return pInstance;
}
